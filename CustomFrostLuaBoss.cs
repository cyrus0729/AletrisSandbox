...
public sealed class ModifiedLuaBadelineBoss : FinalBoss
{
    ...
    private float defaultWaveStrength;
    private float IdleAudioVol;
    private float BosswaveFrequency;
    private float ChangedMusicVol;
    private float ShotChargeVol;
    private float BeamChargeVol;
    private float ShotShootVol;
    ...
    public void ShatterSpinners(LuaTable? args = null)
    {
        ...
        if (args.GetOrDefault<object?>("types", null) is LuaTable typesStringList)
        {
            ...
            {
                if (API.EntityNameToTypeOrNull(typeName) is { } t)
                {
                    types[i++] = t;
                }
                else
