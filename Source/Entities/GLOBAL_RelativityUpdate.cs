...
public class GLOBAL_RelativityUpdate : Entity
{
    ...
    {
        var enableRelativisticVel = AletrisSandboxModule.Session.RelativisticVelocityEnabled ||
        ...
        if (enableRelativisticVel)
        {
            ...
            {
                Logger.Log(LogLevel.Debug, nameof(AletrisSandboxModule), "Clamped Speed:" + speedcap / Math.Abs(playr.Speed.X));
                Logger.Log(LogLevel.Debug, nameof(AletrisSandboxModule), "Relativistic TimeRate:" + Math.Clamp(speedcap / Math.Abs(playr.Speed.X), 0f, 1.0f));
