...
public class InteractiveEntity : NPC
{
    ...
    readonly Sprite mainSprite = GFX.SpriteBank.Create("InteractiveEntity");
    readonly Sprite playerAnims = GFX.SpriteBank.Create("PlayerInteract");

    readonly string plrAnim;
    readonly string entityAnim;
    readonly string entityTalkAnim;
    ...
    {
        Collider = new Hitbox(16, 8);
        Depth = -1;

        flag = data.Attr("flag");
        talkDuration = data.Float("TalkDuration", 2f);
        removeHair = data.Bool("MakeHairInvisible");
        playerOffset = data.Float("Spacing");
        ...
        Add(talker = new(new(-25, 0, 64, 8), !indicator ? new(0f, -99999f) : new Vector2(2f, -4f), Interact));
    ...
    {
        Add(petCoro = new(Petting(player)));
    }

    IEnumerator Petting(Player player)
    {
